...
function getAllModels(config: Config) {
  ...
  for (const provider of config.Providers) {
    for (const model of provider.models) {
      const modeTag = provider.noServer ? ' [direct]' : '';
      models.push({
        name: `${BOLDCYAN}${provider.name}${RESET} → ${CYAN} ${model}${modeTag}`,
        value: `${provider.name},${model}`,
        description: `\n${BOLDCYAN}Provider:${RESET} ${provider.name}${provider.noServer ? ' (direct, no router)' : ''}`,
