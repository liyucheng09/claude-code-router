  ...
  private initializeFromProvidersArray(providersConfig: ConfigProvider[]) {
    providersConfig.forEach((providerConfig: ConfigProvider) => {
      try {
        ...
        // noServer providers don't need api_base_url/api_key (they bypass the router)
        if (!providerConfig.noServer && (!providerConfig.api_base_url || !providerConfig.api_key)) {
          return;
        }
